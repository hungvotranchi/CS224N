...
def negSamplingLossAndGradient(
    ...
    negative_sample = outsideVectors[indices,:]
    negative_sample[1:] *=-1

    score = sigmoid(negative_sample @ centerWordVec)
    loss = -np.sum(np.log(score))
    dout = score - 1
    gradCenterVec = dout @ negative_sample
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    centerWordVecs = centerWordVectors[centerWordIdx]
    for word in outsideWords:
        loss_w, grad_cw, grad_ow = word2vecLossAndGradient(centerWordVecs, word2Ind[word], outsideVectors, dataset)
        loss+= loss_w
        gradCenterVecs[centerWordIdx]+= grad_cw
        gradOutsideVectors += grad_ow
    ### END YOUR CODE
