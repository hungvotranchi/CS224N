...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn = nn.Conv1d(kernel_size=2, padding="same",in_channels=embed_size, out_channels= embed_size)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        last_hidden = last_hidden.transpose(0, 1)  # Shape: (b, 2, h)

        # Concatenate along the hidden size dimension
        # Result shape: (b, 2*h)
        last_hidden_concat = torch.cat((last_hidden[:, 0, :], last_hidden[:, 1, :]), dim=1)
        init_decoder_hidden = self.h_projection(last_hidden_concat)
        
        last_cell = last_cell.transpose(0, 1)  # Shape: (b, 2, h)

        # Concatenate along the hidden size dimension
        # Result shape: (b, 2*h)
        last_cell_concat = torch.cat((last_cell[:, 0, :], last_cell[:, 1, :]), dim=1)
        init_decoder_cell = self.h_projection(last_cell_concat)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1):
            ...
            (cell, state), o_t, _ = self.step(Ybar_t, dec_state= dec_state, enc_hiddens= enc_hiddens, enc_hiddens_proj= enc_hiddens_proj, enc_masks= enc_masks)
